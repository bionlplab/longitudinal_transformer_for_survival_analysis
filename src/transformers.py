...
class TransformerEncoder(Module):
    ...
    def forward(
        ...
        for mod in self.layers:
            ...
            output, attn_map = mod(output, src_mask=mask, is_causal=is_causal,
                                   src_key_padding_mask=src_key_padding_mask_for_layers, need_weights=need_weights)
...
class TransformerDecoder(Module):
    ...
    def forward(self, tgt: Tensor, memory: Tensor, tgt_mask: Optional[Tensor] = None,
        ...
        attn_maps_mha = []
        for mod in self.layers:
            output, attn_map_sa, attn_map_mha = mod(output, memory, tgt_mask=tgt_mask,
...
class TransformerDecoderLayer(Module):
    ...
    def forward(
        ...
        x = tgt
        if self.norm_first:
            _x, attn_map_sa = self._sa_block(self.norm1(x), tgt_mask, tgt_key_padding_mask, tgt_is_causal)
            x = x + _x
            _x, attn_map_mha = self._mha_block(self.norm2(x), memory, memory_mask, memory_key_padding_mask, memory_is_causal)
            x = x + _x
            x = x + self._ff_block(self.norm3(x))
        else:
            _x, attn_map_sa = self._sa_block(x, tgt_mask, tgt_key_padding_mask, tgt_is_causal)
            x = self.norm1(x + _x)
